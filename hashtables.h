...
void bt(std::vector <int>& c, std::vector <int>& q, std::unordered_set <int>& sums, int idx, int sum) {
	...
	if (idx == c.size()) {			// ≈сли дошли максимального количества, то на этом уровне стека 
		if (sum) sums.insert(sum);		// добавл€ем в сет сумму и выходим из рекурсии
		std::cout << "lvl=" << lvl << " sum=" << sum << std::endl;
		lvl--;
		//std::cout << "out lvl(" << lvl << ")" << std::endl;
		return;
	}
	for (int i = 0; i <= q[idx]; ++i) {					// «апускаем некоторое количество раз функцию на этом уровне стека, i - определ€ет сколько раз в наборе будет встречатьс€ элемент
		std::cout << c[idx] * i << "\t" << "lvl=" << lvl << " bt(c,q,s," << idx << "+1" << "," << sum << "+c[" << idx << "]*" << i << ")" << std::endl;
		bt(c, q, sums, idx + 1, sum + c[idx] * i);		// — каждым новым запуском прибавл€ем idx + 1 ( что в один момент даст размер массива монет)
	}
	lvl--;
	//std::cout << "out lvl(" << lvl << std::endl;
}
...
void PermutationWithRepetition(const int* pIn, int N, int K)
{
	...
	int* pOut = new int[K + 1]; // строка из K символов плюс 1 символ дл€ терминального 0
	...
	for (;;)                      // цикл псевдорекурсии
	{
		while (n < N)
		{
			...
			else
			{
				if (n < N)
				{
					*pTop++ = k;          // сохран€ем k и n в стеке
					*pTop++ = n;
				}
...
void chainTraversal(std::unordered_map<int, std::unordered_set<int>>& map, std::unordered_set<int>& set, std::unordered_set<int>& visited, int cur) {
	for (auto& item : map[cur]) {
		if (visited.find(item) == visited.end()) {
			visited.insert(item);
			set.insert(item);
			chainTraversal(map, set, visited, item);
		}
	}
}
...
std::string swapLexOrder(std::string str, std::vector<std::vector<int>> pairs) {
	...
	for (auto& item : map) {
		if (visited.find(item.first) == visited.end()) {
			chainTraversal(map, set, visited, item.first);
		}
		if (!set.empty())
			vSet.push_back(set);
		set.clear();
